...
AR-015 timing shows the same failing 87.102 ns combinational MULDIV baseline
for both capacities, so that problem is independent of the map. AR-017's
iterative-divider follow-up passes 25/50 MHz OOC checks for both profiles;
physical board closure is still separate.

This table is frozen for the first FreeRTOS milestone. Phase 2 must consume the
generated constants and migrate RTL, ACT4, linker, image generation, and tests
together rather than embedding independent copies.
...
These criteria were satisfied on 2026-08-01 by accepting AR-009 and recording
the end-to-end boundary in AR-016. Functional implementation and verification
remain Phase 2 exit criteria.

## Machine-readable configuration boundary

The accepted map is represented by
...
duplication. They record acceptance but do not implement address decoding.

Keep the responsibilities separate:

```text
machine-readable map = cross-language accepted architectural contract
