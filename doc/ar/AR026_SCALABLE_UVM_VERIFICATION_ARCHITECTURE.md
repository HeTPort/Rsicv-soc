...
**State:** Accepted; U0 toolchain sub-gate verified, passive environment not started
...
The toolchain sub-gate was verified on 2026-09-16. ModelSim SE-64 2019.2
explicitly compiled and loaded its installed UVM 1.2 library, constructed a
registered `uvm_test`, executed `run_phase`, emitted the exact success marker,
reported zero UVM errors/fatals, and returned zero. A deliberate false-marker
run returned exit 3, proving that a clean simulator exit alone cannot pass the
runner. Exact commands, hashes, limitations, and traceability are in the
[`u0-passive-uvm` evidence package](../plans/u0-passive-uvm/results.md).

This evidence proves only technical UVM 1.2 availability on the tested host.
It does not independently audit license entitlement or portability and does
not implement the passive DUT interface, transactions, monitor, scoreboard,
coverage, ISS, RAL, or generated workloads. Existing `commit_pkt_t`, committed
`tohost`, directed smoke, ACT4, and regression-result tests still justify the
selected first DUT seam but do not count as UVM integration evidence.
