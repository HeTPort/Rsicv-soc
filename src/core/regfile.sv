...
module regfile (
  ...
  output logic [riscv_pkg::DW-1:0] rs1_rdata_o,
  input  wire logic [4:0]    rs2_raddr_i,
  output logic [riscv_pkg::DW-1:0] rs2_rdata_o,
  input  wire logic          rd_wen_i,
  input  wire logic [4:0]    rd_waddr_i,
  input  wire logic [riscv_pkg::DW-1:0] rd_wdata_i
);
  logic [riscv_pkg::DW-1:0] regs [0:31];
  integer i;
  always_comb begin
    if (rs1_raddr_i == 5'd0)
      rs1_rdata_o = '0;
