...
module core_bus_data_ram #(
  ...
  localparam int REQ_COUNT_W =
      REQ_WAIT_CYCLES > 0 ? $clog2(REQ_WAIT_CYCLES) : 1;
  localparam int RSP_COUNT_W =
      RSP_WAIT_CYCLES > 0 ? $clog2(RSP_WAIT_CYCLES + 1) : 1;

  // Address alignment parameters
  localparam int STRB_W = DW/8;
  localparam int ADDR_SHIFT = (STRB_W > 1) ? $clog2(STRB_W) : 0;
  ...
  // Shift byte address to word address for RAM
  assign ram_addr    = active_req.addr >> ADDR_SHIFT;
    ...
    if (!rst_ni) begin
      ...
      unique case (state_q)
        ADAPTER_IDLE: begin
          if (req_valid_i) begin
            ...
            if (REQ_WAIT_CYCLES == 0) begin
              ...
              req_wait_q <= REQ_COUNT_W'(REQ_WAIT_CYCLES - 1);
        ...
        ADAPTER_REQ_WAIT: begin
          // Standard bus protocol requires req_valid to remain high 
          // until handshake occurs. 
          if (accept) begin
            state_q    <= ADAPTER_RSP_WAIT;
            rsp_wait_q <= RSP_COUNT_W'(RSP_WAIT_CYCLES);
        ...
        ADAPTER_RSP_WAIT: begin
          // Synchronous RAM read data is valid in this cycle
...
`ifndef SYNTHESIS
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      // Assert req_valid must remain high if not accepted
      assert (!(stalled_q && !req_valid_i))
        else $error("req_valid dropped before handshake (protocol violation)");
